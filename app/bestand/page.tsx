...
import BestandClient from "./BestandClient";
...
export default async function BestandPage() {
  // Always use stored defaultFrom (no URL param)
  const settings = await prisma.inventorySettings.findUnique({ where: { id: "default" } });
  const settingsFromISO = settings?.defaultFrom
    ? settings.defaultFrom.toISOString().slice(0, 10)
    : null;
  const fromISO = settingsFromISO ?? todayISO_UTC();
  const fromDate = isoToDateUTC(fromISO);
  const fromMs = fromDate.getTime();

  const lastUpdatedAt = settings?.updatedAt?.toISOString() ?? null;
  ...
  function isLegacyNonColorStart(s: { baseType: string; category: string; size: string }) {
    return (
      s.size === "-" &&
      (s.baseType === "Hilfsmittel" || s.baseType === "Ware" || s.baseType === "Verpackung")
    );
  }

  function legacyToNewCategory(legacyBaseType: string, legacyItemName: string) {
    if (legacyBaseType === "Hilfsmittel" && legacyItemName.startsWith("Paste "))
      return "Pigmente";
  ...
  const shippingItems: Array<{ baseType: string; category: string; size: string }> = [
    ...
    { baseType: "Rührstab", category: "Versandmaterial", size: "-" },
    { baseType: "Klammern", category: "Versandmaterial", size: "-" },
    { baseType: "Versand-Flyer", category: "Versandmaterial", size: "-" },
    ...
    { baseType: "Papier Polster 1 Liter", category: "Versandmaterial", size: "-" },
  ...
  const labelNames: string[] = [
    "Bali Blue", "Blue Ivy", "Ceramic Studio", "Champagne", "Cloud Nine",
    "Coffee Date", "Cosmopolitan", "Cosy Cashmere", "Creamy Silk", "Daily Detox",
    "Don't Send Nudes", "Fashion Week", "Glory Wall", "Golden Hour", "Hakuna Matata",
    "Heartless", "Holly Wood", "Honeymoon", "Hotline Pink", "Hygge",
    "Innocent Wife", "Juicy Orange", "Kiss Me Now", "Light Venus", "Mister Gray",
    "Morning Sun", "No Brainer", "Number 69", "Olive Garden", "Pool Party",
    "Powder Skin", "Power Nap", "Purple Rain", "Run Forest", "Saint-Tropez",
    "Salted Caramel", "Skinny Jeans", "Smokey Eyes", "Soft Avocado", "Space Cake",
    "Summer Mint", "Sushi Lover", "Talk Dirty", "Tea Room", "The Grey Gatsby",
    "The Rock", "Tree House", "True Emotions", "Wabi-Sabi", "Yoga Class",
    "Myx + Match", "Wall Primer", "Pure White", "Ultra White", "Love me Lavender",
    "Soft Sakura", "Neutral Nude", "Mintfulness", "give warmth!", "live simple!",
    "come closer!", "feel jade!", "dive deep!", "pay Attention!", "stay together!",
    "be rooted!", "taste south!",
  ];

  const labelItems: Array<{ baseType: string; category: string; size: string }> =
    labelNames.flatMap((name) => [
  ...
  return (
    <main style={{ padding: 24, fontFamily: "system-ui, -apple-system, Segoe UI, Roboto" }}>
      <h1 style={{ fontSize: 22, marginBottom: 10 }}>Bestand</h1>

      {/* Dashboard – identical layout to /inventory */}
      <div
        style={{
          padding: 12,
            ...
            {low.length === 0 ? (
              ...
              <ul style={{ margin: 0, paddingLeft: 16 }}>
                {low
                  ...
                  .map((r, idx) => (
                    <li
                      key={`${r.baseType}|${r.category}|${r.size}|${idx}`}
